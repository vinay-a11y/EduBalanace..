    ...
    if (!loansSnapshot.empty) {
        ...
        document.querySelectorAll(".pay-btn").forEach(button => {
            button.addEventListener("click", (event) => {
                ...
                window.location.href = `/htmls/pay.html?loanId=${loanId}`; // Correct URL syntax
            });
        });
        ...
        document.querySelectorAll(".transaction-btn").forEach(button => {
            button.addEventListener("click", (event) => {
                const loanId = event.target.getAttribute("data-loan-id");
                window.location.href = `/htmls/transaction.html?loanId=${loanId}`; // Correct URL syntax
            });
        });
        ...
        document.querySelectorAll(".details-btn").forEach(button => {
            button.addEventListener("click", (event) => {
                const loanId = event.target.getAttribute("data-loan-id");
                window.location.href = `/htmls/loandet.html?loanId=${loanId}`; // Correct URL syntax
            });
        });
